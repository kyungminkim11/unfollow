...
- **관리자 대시보드**: 이메일 매직링크 인증, 구독자·수요 통계, 검색, CSV, 삭제
...
캠페인 초안, 미리보기와 대기열은 발송 제공자 연결 없이 사용할 수 있습니다. 실제 이메일 발송에는 Supabase Edge Function 환경변수가 필요합니다.
...
발송 연결이 없으면 관리자 화면에 `연결 필요`로 표시되며 실제 메일은 전송되지 않습니다. 발송은 50건 단위 배치로 처리하고, 캠페인별 성공·실패·남은 건수를 저장합니다.
...
모든 업무 테이블은 RLS를 사용하고 공개 직접 접근 정책을 두지 않습니다. 공개 신청은 검증된 Edge Function으로만 처리하며 관리자 조회·삭제·발송은 유효한 JWT와 허용 이메일 확인을 모두 통과해야 합니다.
