...
공식 JSON ZIP 분석뿐 아니라 Instagram 웹 화면에서 현재 팔로워·팔로잉을 수집하고, 스캔 기록을 비교해 관계 변화를 확인할 수 있습니다.

- 서비스: https://unfollow.lavalabs.co.kr
- 운영: Lava Labs
- 현재 운영: 무료 베타
- 서버로 보내지 않는 정보: Instagram ZIP, 관계 분석 결과, 팔로워·팔로잉 스캔 명단, 스캔 이력, 팔로우 취소 작업 상태
- 지원 환경: 웹 분석은 최신 브라우저, 웹 스캔·변화 추적·작업 실행은 PC Chrome 또는 Edge

## v24 주요 기능

### Instagram 관계 대시보드

Chrome Companion의 현재 스캔과 과거 스캔 기록을 비교합니다.

- 현재 팔로워, 팔로잉, 맞팔, 맞팔 아님, 나를 팔로우만 하는 계정 수
- 이전 시점과 현재 시점 직접 선택
- 나를 언팔했지만 내가 아직 팔로우 중인 계정
- 새 팔로워
- 새 비맞팔
- 새 맞팔
- 내가 새로 팔로우한 계정
- 내가 팔로우를 취소한 계정
- 변화 목록 검색, 프로필 열기와 CSV 저장
- 나를 언팔한 계정·새 비맞팔을 선택해 팔로우 취소 작업으로 연결
- 계정별 최근 스캔 기록을 최대 24개까지 로컬 저장

이전 시점은 현재 시점보다 오래되고 같은 Instagram 계정인 기록만 선택할 수 있습니다. 일부 목록으로 끝난 스캔은 화면에 경고하며, 두 시점이 모두 전체 수집된 경우에만 완전한 변화 비교로 표시합니다.
...
- 웹 스캔, 스캔 이력 또는 ZIP 분석 명단에서 작업 대상 선택
...
- `/` — 관계 대시보드, 현재 관계 스캔, ZIP 분석과 작업 목록
...
- 로그인한 계정의 프로필 아이디
- 팔로워, 팔로잉, 맞팔과 맞팔 아닌 아이디 명단
- 스캔 완료 여부, 시각과 경고
- 계정별 최근 스캔 기록 최대 24개
- 선택 팔로우 취소 큐
- 처리 상태, 오류와 실행 설정

현재 스캔 상태는 `matchalRelationshipStateV23`, 비교 기록은 `matchalRelationshipHistoryV24` 키에 저장합니다. 확장 프로그램 저장 데이터를 삭제하면 함께 제거됩니다. 이 정보는 라바랩스 서버, 문의 또는 뉴스레터 데이터베이스로 전송하지 않습니다.
...
- `storage`: 스캔 명단, 비교 기록, 작업 목록과 진행 상태
- `tabs`: 내 프로필과 선택한 작업 대상 프로필 이동
- `sidePanel`: 관계 대시보드, 스캔과 작업 제어 UI
...
- 두 스캔 시점의 관계 변화 계산 검사
- 나를 언팔한 계정과 새 비맞팔의 작업 큐 연결 검사
- 데스크톱·모바일 관계 대시보드
...
- `extension/sidepanel.js` — 현재 관계 스캔 결과와 작업 실행 UI
- `extension/sidepanel-v24.*` — 로컬 스캔 이력과 최근 변화 대시보드
- `extension/background.js` — 로컬 상태·스캔 이력과 웹 메시지 처리
- `extension/content-bridge.js` — 웹과 Companion 관계 상태·이력 연결
- `assets/relationship-scan-v23.*` — 웹 현재 관계 스캔 결과 UI
- `assets/relationship-dashboard-v24.*` — 웹 관계 요약·시점 비교와 변화 목록 UI
- `assets/automation-v22.*` — ZIP 분석 결과 작업 목록 UI
- `assets/automation-parser-v22.js` — A·B 비교 자동화 대상 계산
- `scripts/relationship-scan-v23-smoke.mjs` — 스캐너·웹 UI 회귀 검사
- `scripts/relationship-dashboard-v24-smoke.mjs` — 관계 대시보드·변화 계산 자동 검사
- `scripts/build-v13.mjs` — 정적 페이지 결합과 v24 자산 검증
- `sw.js` — PWA 캐시와 네트워크 우선 업데이트

## 주의

맞팔체커는 Instagram 또는 Meta가 공식 제공하거나 제휴한 서비스가 아닙니다. Instagram 화면 구조와 정책은 변경될 수 있습니다. 스캔 결과와 변화 목록을 검토하고 처음에는 소수 계정으로 시험한 뒤 사용해야 합니다.