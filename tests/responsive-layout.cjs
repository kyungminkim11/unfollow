...
function countComputedTracks(value) {
...
(async () => {
  ...
  for (const testCase of cases) {
    ...
    await page.goto(`${BASE_URL}?qa=${encodeURIComponent(testCase.name)}`, { waitUntil: 'domcontentloaded', timeout: 60_000 });
    await page.waitForSelector('body.service-v15 .hero.v14Hero', { timeout: 60_000 });
    await page.waitForSelector('.v14HeroAside', { timeout: 60_000 });
    ...
    const metrics = await page.evaluate(() => {
      ...
      const textChecks = Array.from(document.querySelectorAll(
        ...
        return {
          ...
          overflowWrap: computed.overflowWrap,
          whiteSpace: computed.whiteSpace,
          likelyCharacterColumn: text.length >= 3 && rect.width < 48 && estimatedLines >= Math.max(3, Math.floor(text.replace(/\s/g, '').length * 0.55)),
        };
      });
      ...
      return {
        ...
        heroTrackValue: heroStyle?.gridTemplateColumns || null,
    ...
    const trackCount = countComputedTracks(metrics.heroTrackValue);
    const noHorizontalOverflow = metrics.documentWidth.scrollWidth <= metrics.documentWidth.clientWidth + 1;
    const noCharacterColumns = !metrics.textChecks.some(item => item.likelyCharacterColumn || item.writingMode !== 'horizontal-tb');
    const asideWideEnough = testCase.width <= 420
    ...
    const checks = {
      heroColumnCount: { pass: trackCount === testCase.expectedColumns, expected: testCase.expectedColumns, actual: trackCount, value: metrics.heroTrackValue },
      noHorizontalOverflow: { pass: noHorizontalOverflow, ...metrics.documentWidth },
      noCharacterColumns: { pass: noCharacterColumns, offenders: metrics.textChecks.filter(item => item.likelyCharacterColumn || item.writingMode !== 'horizontal-tb') },
      asideWideEnough: { pass: Boolean(asideWideEnough), width: metrics.elements.aside?.width || 0 },
  ...
  fs.writeFileSync(path.join(OUT_DIR, 'summary.json'), JSON.stringify(results.map(({ case: c, checks, metrics }) => ({ case: c, checks, bodyClasses: metrics.bodyClasses, heroInlineStyle: metrics.heroInlineStyle, heroTrackValue: metrics.heroTrackValue })), null, 2));
  await browser.close();
