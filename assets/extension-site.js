(() => {
  ...
  const STORE_URL = '/downloads/matchal-companion-v23.zip';
  ...
  function updateUI() {
    ...
    document.querySelectorAll('[data-extension-status]').forEach(element => {
      element.textContent = state.ready ? `연결됨${state.version ? ` · v${state.version}` : ''}` : '설치 후 팔로워·팔로잉 웹 스캔을 사용할 수 있어요';
    });
