(() => {
  ...
  const STORE_URL = '/downloads/matchal-companion-v24.zip';
  ...
  function createGuideDialog() {
    ...
    backdrop.innerHTML = `<div class="extensionGuideDialog" tabindex="-1"><div class="extensionGuideHead"><div><h2 id="extensionGuideTitle">맞팔체커 Companion 사용법</h2><p>현재 관계는 Instagram 웹 스캔으로 확인하고, 두 번째 스캔부터 최근 변화가 자동으로 쌓입니다.</p></div><button type="button" class="extensionGuideClose" data-extension-close aria-label="닫기">${svg.close}</button></div><div class="extensionGuideBody"><div class="extensionGuideState" data-guide-state><span class="stateDot"></span><span>확장 프로그램 설치 여부를 확인하고 있습니다.</span></div><div class="extensionGuideSteps"><div class="extensionGuideStep"><span class="extensionGuideStepNum">1</span><div><strong>Instagram 로그인과 내 아이디 확인</strong><p>PC Chrome에서 Instagram에 로그인하고 Companion 사이드패널을 엽니다.</p></div></div><div class="extensionGuideStep"><span class="extensionGuideStepNum">2</span><div><strong>팔로워·팔로잉 관계 스캔</strong><p>내 프로필의 두 목록을 순서대로 스크롤해 팔로워, 팔로잉, 맞팔과 비맞팔을 계산합니다.</p></div></div><div class="extensionGuideStep"><span class="extensionGuideStepNum">3</span><div><strong>두 번째 스캔부터 변화 확인</strong><p>최근 스캔 기록을 비교해 나를 언팔한 계정, 새 팔로워, 새 비맞팔과 새 맞팔을 보여줍니다.</p></div></div><div class="extensionGuideStep"><span class="extensionGuideStepNum">4</span><div><strong>선택한 변화만 작업으로 연결</strong><p>나를 언팔했지만 내가 아직 팔로우 중인 계정이나 새 비맞팔을 팔로우 취소 목록으로 만들 수 있습니다.</p></div></div></div><div class="extensionGuideNotice" data-install-notice>ZIP을 내려받아 압축을 푼 뒤 chrome://extensions에서 개발자 모드를 켜고 ‘압축해제된 확장 프로그램을 로드합니다’를 선택하세요.</div><div class="extensionGuideActions"><a class="extensionPromoBtn" href="https://accountscenter.instagram.com/info_and_permissions/dyi/" target="_blank" rel="noopener noreferrer">${svg.download}<span>과거 비교용 공식 데이터</span></a><button type="button" class="extensionPromoBtn primary" data-extension-primary>${svg.extension}<span class="install-label">Companion ZIP 받기</span><span class="open-label">사이드패널 열기</span></button></div><p class="extensionGuideFoot">스캔 기록은 이 Chrome 확장 프로그램의 로컬 저장소에만 보관됩니다. 비밀번호·인증 코드·쿠키·세션 토큰을 읽지 않습니다.</p></div></div>`;
  ...
  function updateUI() {
    ...
    document.querySelectorAll('[data-extension-status]').forEach(element => {
      element.textContent = state.ready ? `연결됨${state.version ? ` · v${state.version}` : ''}` : '설치 후 관계 스캔과 변화 추적을 사용할 수 있어요';
    });
