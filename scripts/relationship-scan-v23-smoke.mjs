...
try{
  ...
  const scannerPage=await browser.newPage({viewport:{width:1000,height:800}});
  await scannerPage.goto(`${baseURL}/`,{waitUntil:'domcontentloaded'});
  await scannerPage.evaluate(()=>{
    ...
    window.chrome={runtime:{onMessage:{addListener(listener){window.__scannerListener=listener;}},sendMessage(){return Promise.resolve({ok:true});}}};
  });
