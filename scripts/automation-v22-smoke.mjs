...
try{
  for(const viewport of [{width:1280,height:900,label:'desktop'},{width:390,height:844,label:'mobile'}]){
    ...
    await page.waitForFunction(()=>document.querySelector('[data-source-count="current_non_mutual"]')?.textContent.trim()==='2');
    const metrics=await page.evaluate(()=>({
      section:document.querySelector('#automationV22')?.getBoundingClientRect(),
    ...
    await page.waitForFunction(()=>window.__capturedQueue?.items?.length===2);
    ...
    check(`${viewport.label} 자동화 UI 접근성`,serious.length===0,{violations:serious.map(item=>item.id)});
