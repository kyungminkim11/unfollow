...
async function inspectMobile(browser,width,height,label){
  ...
  const metrics=await page.evaluate(()=>{
    ...
    const oldNavHidden=Array.from(document.querySelectorAll('.v15MobileNav,.bottomNavV8')).every(element=>{
  ...
  check(`${label} 오프라인 안내 비가림`,offline.text.includes('오프라인 모드')&&offline.top>=60&&offline.bottom<offline.navTop&&offline.height<60,offline);
